document.addEventListener('DOMContentLoaded', () => {
  ...
  function renderStampCard(containerEl, totalPoints) {
    ...
    for (let i = 0; i < 10; i++) {
      html += `<div class="kiosk-stamp${i < filled ? ' filled' : ''}">${i < filled ? '♦' : ''}</div>`;
    }
    html += '</div>';

    let note = '';
    if (completedBlocks > 0) {
      note += `💎 You have <strong>$${completedBlocks * 10}</strong> in rewards ready — ask us to redeem it! `;
    }
  ...
  function refreshRedeemPointsBlock() {
    ...
    if (redeemableAmount >= 10) {
      redeemPointsLabel.textContent = `💎 Use $${redeemableAmount} in reward points on this visit`;
    ...
    try {
      ...
      if (pointsRedeemedEl) {
        if (data.booking.points_redeemed_error) {
          ...
          pointsRedeemedEl.textContent = `Reward points applied: -$${data.booking.points_redeemed_amount.toFixed(2)}`;
