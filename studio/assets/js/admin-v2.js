...
async function uploadImage(file, subdir, csrfToken) {
  ...
  const res = await fetch('/studio/actions/upload-image.php', { method: 'POST', body: fd });
  return res.json();
}
...
async function notifPoll(isInitial) {
  ...
  try {
    const res = await fetch('/studio/actions/activity-feed.php?since=' + encodeURIComponent(since));
...
async function updateBookingStatus(bookingId, status, csrfToken) {
  try {
    const data = await postJSON('/studio/actions/update-booking-status.php', {
