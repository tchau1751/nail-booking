...
$allStaff = [
  ['id' => '1', 'name' => 'Maria', 'color' => '#f8b4d8'],
  ['id' => '2', 'name' => 'Susan', 'color' => '#b4d8f8'],
  ['id' => '3', 'name' => 'Laura', 'color' => '#d8b4f8'],
  ['id' => '4', 'name' => 'Anna', 'color' => '#b4f8d8'],
  ['id' => '5', 'name' => 'Emma', 'color' => '#f8d8b4'],
  ['id' => '6', 'name' => 'Grace', 'color' => '#d8f8b4'],
];

$selectedStaff = isset($_GET['staff']) ? explode(',', $_GET['staff']) : ['1', '2', '3', '4'];

function getStaffByID($id, $allStaff) {
...
<head>
  ...
  <style>
    * { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    .nav-buttons button { padding: 8px 12px; background: white; color: #c9947f; border: none; border-radius: 4px; cursor: pointer; font-weight: 600; font-size: 12px; }
    ...
    .controls-title { font-weight: 700; margin-bottom: 12px; font-size: 14px; }
    ...
    .day-grid { display: grid; grid-template-columns: 70px repeat(auto-fit, minmax(180px, 1fr)); gap: 0; border: 1px solid #ddd; }
    .day-header { background: #f0f0f0; padding: 12px; font-weight: 700; text-align: center; border-right: 1px solid #ddd; border-bottom: 3px solid #c9947f; font-size: 12px; }
    ...
    .week-header { background: #f0f0f0; padding: 12px; font-weight: 700; text-align: center; border-right: 1px solid #ddd; border-bottom: 3px solid #c9947f; font-size: 12px; }
    ...
    .month-header { background: #f0f0f0; padding: 12px; font-weight: 700; text-align: center; border-right: 1px solid #ddd; border-bottom: 1px solid #ddd; font-size: 12px; }
    .month-header:last-child { border-right: none; }
    .month-day { border-right: 1px solid #ddd; border-bottom: 1px solid #ddd; min-height: 100px; padding: 8px; background: white; position: relative; }
...
<body>
    ...
    <div class="header">
      ...
      <div class="header-controls">
        ...
        <div class="nav-buttons">
          ...
          <div class="date-display" id="dateDisplay"><?= $dateObj->format('M j, Y') ?></div>
          <button onclick="navigate(1)">Next →</button>
          <button onclick="goToday()">Today</button>
        </div>
        <button style="background: #059669; color: white; padding: 8px 16px; border: none; border-radius: 4px; cursor: pointer; font-weight: 600; font-size: 12px;" onclick="openModal()">+ New Booking</button>
      </div>
    </div>
      ...
      <div id="dayView" style="display: <?= $view === 'day' ? 'block' : 'none' ?>;">
        <div class="day-grid">
          ...
          <?php foreach ($selectedStaff as $sid):
            $staff = getStaffByID($sid, $allStaff);
            if ($staff): ?>
  ...
  <script>
    ...
    function updateStaff() {
      ...
      location.href = '?date=<?= $date ?>&view=<?= $view ?>&staff=' + (checked || '1');
    }
