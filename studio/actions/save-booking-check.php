...
try {
    if ($id > 0) {
        ...
        // Send SMS confirmation to client if phone number exists
        if ($phone !== '') {
            sendBookingConfirmationSMS($phone, $fullName, $dateObj, $timeObj);
        }
    }
} catch (Exception $e) {
...
function sendBookingConfirmationSMS($phone, $clientName, $dateObj, $timeObj) {
    ...
    $appointmentDate = $dateObj->format('M d, Y');
    $appointmentTime = $timeObj->format('g:i A');

    $message = "Hi $clientName, your appointment at Diamond Nails & Spa has been booked for $appointmentDate at $appointmentTime. Thank you!";
