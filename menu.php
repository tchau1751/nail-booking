<?php
require_once __DIR__ . '/includes/functions.php';

$business = get_business_settings();

$pdo = get_db();
$categories = $pdo->query('SELECT * FROM menu_categories ORDER BY sort_order ASC, id ASC')->fetchAll();
$itemsByCategory = [];
if (!empty($categories)) {
    $items = $pdo->query('SELECT * FROM menu_items ORDER BY category_id ASC, sort_order ASC, id ASC')->fetchAll();
    foreach ($items as $item) {
        $itemsByCategory[(int) $item['category_id']][] = $item;
    }
}
?>
<!DOCTYPE html>
<html lang="en">
<head>
<meta charset="UTF-8">
<meta name="viewport" content="width=device-width, initial-scale=1.0">
<title>Price Menu — <?= e($business['business_name']) ?></title>
<meta name="description" content="Full price menu for <?= e($business['business_name']) ?> — manicures, pedicures, waxing, nail enhancements, and more.">
<link rel="icon" type="image/svg+xml" href="/assets/img/favicon.svg">
<link rel="icon" type="image/png" sizes="32x32" href="/assets/img/favicon-32.png">
<link rel="icon" type="image/png" sizes="16x16" href="/assets/img/favicon-16.png">
<link rel="apple-touch-icon" href="/assets/img/apple-touch-icon.png">
<link rel="preconnect" href="https://fonts.googleapis.com">
<link rel="preconnect" href="https://fonts.gstatic.com" crossorigin>
<link href="https://fonts.googleapis.com/css2?family=Playfair+Display:ital,wght@0,500;0,600;0,700;1,500&family=Poppins:wght@300;400;500;600;700&display=swap" rel="stylesheet">
<link rel="stylesheet" href="/assets/css/style-v4.css">
<style>
  .menu-hero { padding: 160px 0 60px; text-align: center; }
  .menu-hero h1 { font-family:'Playfair Display', serif; font-size: clamp(32px, 5vw, 52px); color: var(--ink); margin: 0 0 12px; }
  .menu-hero p { color: var(--ink-soft); font-size: 16px; max-width: 560px; margin: 0 auto; }
  .menu-wrap { padding: 20px 0 100px; }
  .menu-category { margin-bottom: 56px; }
  .menu-category h2 { font-family:'Playfair Display', serif; font-size: 26px; color: var(--rose-gold); text-transform: uppercase; letter-spacing: 0.03em; margin: 0 0 4px; }
  .menu-category .menu-subtitle { font-size: 14px; color: var(--ink-soft); margin-bottom: 14px; font-style: italic; }
  .menu-category-rule { border: none; border-top: 1px solid rgba(184,131,106,0.3); margin: 0 0 20px; }
  .menu-item { margin-bottom: 14px; }
  .menu-item-row { display: flex; align-items: baseline; gap: 8px; }
  .menu-item-name { font-weight: 600; color: var(--ink); font-size: 15.5px; white-space: nowrap; }
  .menu-item-leader { flex: 1; border-bottom: 1px dotted rgba(58,44,38,0.35); position: relative; top: -4px; }
  .menu-item-price { font-weight: 700; color: var(--ink); font-size: 15.5px; white-space: nowrap; }
  .menu-item-note { font-weight: 600; color: var(--ink); font-size: 15px; }
  .menu-item-desc { font-size: 13.5px; color: var(--ink-soft); line-height: 1.65; margin-top: 4px; white-space: pre-line; }
  .menu-cta { text-align: center; margin-top: 20px; }
</style>
</head>
<body>

<nav class="navbar is-scrolled">
  <div class="container">
    <a href="/booking-login.php" class="brand">
      <?php if (!empty($business['logo_url'])): ?>
        <img src="<?= e($business['logo_url']) ?>" alt="<?= e($business['business_name']) ?>" style="height:52px;width:auto;">
      <?php else: ?>
        <span class="brand-mark">D</span>
        <span class="brand-name"><?= e($business['business_name']) ?><span>Nail Studio &amp; Spa</span></span>
      <?php endif; ?>
    </a>
    <div class="nav-links">
      <a href="/booking-login.php#services">Services</a>
      <a href="/menu.php">Menu</a>
      <a href="/booking-login.php#gallery">Gallery</a>
      <a href="/booking-login.php#about">About</a>
      <a href="/booking-login.php#booking">Book Now</a>
    </div>
    <div class="nav-actions">
      <a href="tel:<?= e(preg_replace('/[^0-9+]/', '', $business['business_phone'])) ?>" class="nav-phone">
        <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M22 16.92v3a2 2 0 0 1-2.18 2 19.79 19.79 0 0 1-8.63-3.07 19.5 19.5 0 0 1-6-6 19.79 19.79 0 0 1-3.07-8.67A2 2 0 0 1 4.11 2h3a2 2 0 0 1 2 1.72c.127.96.361 1.903.7 2.81a2 2 0 0 1-.45 2.11L8.09 9.91a16 16 0 0 0 6 6l1.27-1.27a2 2 0 0 1 2.11-.45c.907.339 1.85.573 2.81.7A2 2 0 0 1 22 16.92z"/></svg>
        <?= e($business['business_phone']) ?>
      </a>
      <a href="/booking-login.php#booking" class="btn btn-primary">Book Your Appointment</a>
      <button class="nav-toggle" aria-label="Open menu" onclick="document.querySelector('.mobile-menu').classList.add('open')">
        <svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M3 12h18M3 6h18M3 18h18"/></svg>
      </button>
    </div>
  </div>
</nav>

<div class="mobile-menu">
  <button class="mobile-menu-close" aria-label="Close menu">&times;</button>
  <a href="/booking-login.php#services">Services</a>
  <a href="/menu.php">Menu</a>
  <a href="/booking-login.php#gallery">Gallery</a>
  <a href="/booking-login.php#about">About</a>
  <a href="/booking-login.php#booking">Book Now</a>
</div>

<section class="menu-hero">
  <div class="container">
    <h1>Price Menu</h1>
    <p>Every treatment, every add-on. Prices may vary slightly based on nail length and condition.</p>
  </div>
</section>

<section class="menu-wrap">
  <div class="container" style="max-width: 760px;">
    <?php if (empty($categories)): ?>
      <p style="text-align:center;color:var(--ink-soft);">Our price menu is being updated — please call <?= e($business['business_phone']) ?> for current pricing.</p>
    <?php else: ?>
      <?php foreach ($categories as $cat): ?>
        <div class="menu-category">
          <h2><?= e($cat['name']) ?></h2>
          <?php if (!empty($cat['subtitle'])): ?><div class="menu-subtitle"><?= e($cat['subtitle']) ?></div><?php endif; ?>
          <hr class="menu-category-rule">
          <?php foreach ($itemsByCategory[(int) $cat['id']] ?? [] as $item): ?>
            <div class="menu-item">
              <?php if ($item['price_label']): ?>
                <div class="menu-item-row">
                  <span class="menu-item-name"><?= e($item['name']) ?></span>
                  <span class="menu-item-leader"></span>
                  <span class="menu-item-price"><?= e($item['price_label']) ?></span>
                </div>
              <?php else: ?>
                <div class="menu-item-note"><?= e($item['name']) ?></div>
              <?php endif; ?>
              <?php if (!empty($item['description'])): ?>
                <div class="menu-item-desc"><?= e($item['description']) ?></div>
              <?php endif; ?>
            </div>
          <?php endforeach; ?>
        </div>
      <?php endforeach; ?>
    <?php endif; ?>
    <div class="menu-cta">
      <a href="/booking-login.php#booking" class="btn btn-primary">Book Your Appointment</a>
    </div>
  </div>
</section>

<!-- ================= FOOTER ================= -->
<footer class="footer">
  <div class="container">
    <div class="footer-grid">
      <div class="footer-brand">
        <span class="brand-name" style="color:#fff;"><?= e($business['business_name']) ?></span>
        <p>A premium nail studio in Layton, Utah — manicures, pedicures, nail art, and acrylics delivered with genuine care.</p>
        <div class="footer-social">
          <?php if (!empty($business['instagram_url'])): ?>
          <a href="<?= e($business['instagram_url']) ?>" aria-label="Instagram" target="_blank" rel="noopener">
            <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><rect x="2" y="2" width="20" height="20" rx="5"/><circle cx="12" cy="12" r="4"/><circle cx="17.5" cy="6.5" r="1"/></svg>
          </a>
          <?php endif; ?>
          <?php if (!empty($business['facebook_url'])): ?>
          <a href="<?= e($business['facebook_url']) ?>" aria-label="Facebook" target="_blank" rel="noopener">
            <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M18 2h-3a5 5 0 0 0-5 5v3H7v4h3v8h4v-8h3l1-4h-4V7a1 1 0 0 1 1-1h3z"/></svg>
          </a>
          <?php endif; ?>
        </div>
      </div>
      <div>
        <h4>Explore</h4>
        <p><a href="/booking-login.php#services">Services</a></p>
        <p><a href="/menu.php">Menu</a></p>
        <p><a href="/booking-login.php#gallery">Gallery</a></p>
        <p><a href="/booking-login.php#about">About</a></p>
        <p><a href="/booking-login.php#booking">Book Now</a></p>
      </div>
      <div>
        <h4>Hours</h4>
        <p><?= nl2br(e(str_replace(' · ', "\n", $business['hours_note']))) ?></p>
      </div>
      <div>
        <h4>Visit Us</h4>
        <p><?= e($business['business_address']) ?></p>
        <p><a href="tel:<?= e(preg_replace('/[^0-9+]/', '', $business['business_phone'])) ?>"><?= e($business['business_phone']) ?></a></p>
        <?php if (!empty($business['business_email'])): ?>
        <p><a href="mailto:<?= e($business['business_email']) ?>"><?= e($business['business_email']) ?></a></p>
        <?php endif; ?>
        <p><a href="/privacy-policy.php">Privacy Policy</a></p>
      </div>
    </div>
    <div class="footer-bottom">
      <span>&copy; <?= date('Y') ?> <?= e($business['business_name']) ?>. All rights reserved.</span>
      <span>Layton, Utah</span>
    </div>
  </div>
</footer>

</body>
</html>
